                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Edit Event Item: {{ $event_item->title }}</h4>
                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.event_items.update', $event_item) }}" method="POST">
                                @csrf
                                @method('PUT')
                                    ...
                                    <select name="event_id" class="form-control @error('event_id') is-invalid @enderror"
                                        ...
                                        @foreach ($events as $event)
                                            <option value="{{ $event->id }}"
                                                {{ old('event_id', $event_item->event_id) == $event->id ? 'selected' : '' }}>
                                                {{ $event->title }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-sm-6">
                                    ...
                                    <input type="text" name="title"
                                        ...
                                        value="{{ old('title', $event_item->title) }}" required>
                                    @error('title')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
