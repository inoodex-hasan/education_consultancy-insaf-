...
@section('title', 'Edit Health Item')
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Edit Health Item: {{ $health_item->title }}</h4>
                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.health_items.update', $health_item) }}" method="POST">
                                @csrf
                                @method('PUT')
                                    ...
                                    <select name="health_id" class="form-control @error('health_id') is-invalid @enderror"
                                        ...
                                        @foreach ($healths as $health)
                                            <option value="{{ $health->id }}"
                                                {{ old('health_id', $health_item->health_id) == $health->id ? 'selected' : '' }}>
                                                {{ $health->title }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="title"
                                        ...
                                        value="{{ old('title', $health_item->title) }}" required>
                                    @error('title')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mt-4">
                                    ...
                                    <a href="{{ route('admin.health_items.index') }}"
