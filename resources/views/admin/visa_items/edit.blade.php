                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Edit Visa Item: {{ $visa_item->title }}</h4>
                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.visa_items.update', $visa_item) }}" method="POST">
                                @csrf
                                @method('PUT')

                                <div class="form-group col-sm-6">
                                    <label>Parent Visa Record <span class="text-danger">*</span></label>
                                    <select name="visa_id" class="form-control @error('visa_id') is-invalid @enderror"
                                        ...
                                        @foreach ($visas as $visa)
                                            <option value="{{ $visa->id }}"
                                                {{ old('visa_id', $visa_item->visa_id) == $visa->id ? 'selected' : '' }}>
                                                {{ $visa->title }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-sm-6">
                                    ...
                                    <input type="text" name="title"
                                        ...
                                        value="{{ old('title', $visa_item->title) }}" required>
                                    @error('title')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
